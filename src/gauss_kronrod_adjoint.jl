...
struct ODEGaussKronrodAdjointSensitivityFunction{C <: AdjointDiffCache,
    Alg <: GaussKronrodAdjoint,
    uType, SType, CPS, pType,
    fType,
    GI <: GaussKronrodIntegrand,
...
mutable struct GaussKronrodCheckpointSolution{S, I, T, T2}
...
function ODEGaussKronrodAdjointSensitivityFunction(
        g, sensealg, gaussKronrodint, discrete, sol, dgdu, dgdp,
        f, alg,
    ...
    checkpoint_sol = if checkpointing
        ...
        if tstops === nothing
            ...
            gaussKronrodint.sol = cpsol
        else
            if maximum(interval[1] .< tstops .< interval[2])
                # callback might have changed p
                _p = GaussKronrodreset_p(sol.prob.kwargs[:callback], interval)
                #cpsol = solve(remake(sol.prob, tspan = interval, u0 = sol(interval[1])),
                #    tstops = tstops,
                #    p = _p, sol.alg; tols...)
        ...
        GaussKronrodCheckpointSolution(cpsol, intervals, cursor, tols, tstops)
    else
        nothing
    end
    diffcache, y = adjointdiffcache(
        ...
        quad = true)
    return ODEGaussKronrodAdjointSensitivityFunction(diffcache, sensealg, discrete,
        y, sol, checkpoint_sol, sol.prob, f, gaussKronrodint, integrating_cb)
end

function GaussKronrodfindcursor(intervals, t)
...
# u = λ'
function (S::ODEGaussKronrodAdjointSensitivityFunction)(du, u, p, t)
...
function (S::ODEGaussKronrodAdjointSensitivityFunction)(du, u, p, t, W)
    (; sol, checkpoint_sol, discrete, prob, f) = S
...
function (S::ODEGaussKronrodAdjointSensitivityFunction)(u, p, t)
...
function split_states(du, u, t, S::ODEGaussKronrodAdjointSensitivityFunction; update = true)
    (; sol, y, checkpoint_sol, discrete, prob, f, GaussKronrodInt) = S
    if update
        if checkpoint_sol === nothing
            ...
            if !(interval[1] <= t <= interval[2])
                cursor′ = GaussKronrodfindcursor(intervals, t)
                interval = intervals[cursor′]
...
function split_states(u, t, S::ODEGaussKronrodAdjointSensitivityFunction; update = true)
    (; y, sol) = S
...
@noinline function ODEAdjointProblem(sol, sensealg::GaussKronrodAdjoint, alg,
        GaussKronrodInt::GaussKronrodIntegrand, integrating_cb,
        t = nothing,
    ...
    sense = ODEGaussKronrodAdjointSensitivityFunction(g, sensealg, GaussKronrodInt, discrete, sol,
...
function GaussKronrodreset_p(CBS, interval)
...
function GaussKronrodIntegrand(sol, sensealg, checkpoints, dgdp = nothing)
    prob = sol.prob
    (; f, tspan) = prob
    ...
    GaussKronrodIntegrand(cpsol, p, y, λ, pf, f_cache, pJ, paramjac_config,
        sensealg, dgdp_cache, dgdp)
end

# out = λ df(u, p, t)/dp at u=y, p=p, t=t
function vec_pjac!(out, λ, y, t, S::GaussKronrodIntegrand)
    ...
    elseif sensealg.autojacvec isa MooncakeVJP
        ...
        error("autojacvec choice $(sensealg.autojacvec) is not supported by GaussKronrodAdjoint")
    end
    # TODO: Add tracker?
    return out
end

function (S::GaussKronrodIntegrand)(out, t, λ)
...
function (S::GaussKronrodIntegrand)(t, λ)
    out = allocate_zeros(S.p)
    S(out, t, λ)
end

function _adjoint_sensitivities(sol, sensealg::GaussKronrodAdjoint, alg; t = nothing,
    ...
    integrand = GaussKronrodIntegrand(sol, sensealg, checkpoints, dgdp_continuous)
