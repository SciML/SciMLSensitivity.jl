...
function DiffEqBase._concrete_solve_adjoint(
    ...
    kwargs_fwd = NamedTuple{Base.diff_names(
        Base._nt_names(values(kwargs)), (:callback, :initializealg))}(values(kwargs))
...
function DiffEqBase._concrete_solve_adjoint(
    ...
    _prob = remake(prob,
        f =  f = ODEFunction{isinplace(prob), SciMLBase.FullSpecialize}(unwrapped_f(prob.f)) )
    ...
    tape, result,
    shadow_result = forward(Enzyme.Const(diff_func), Enzyme.Duplicated(copy(u0), du0),
        Enzyme.Duplicated(copy(p), dp))
    ...
    function enzyme_sensitivity_backpass(Δ)
        ...
        reverse(Enzyme.Const(diff_func), Enzyme.Duplicated(u0, du0),
            Enzyme.Duplicated(p, dp), tape)
        if originator isa SciMLBase.TrackerOriginator ||
           originator isa SciMLBase.ReverseDiffOriginator
