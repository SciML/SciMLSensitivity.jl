            ...
            if !isRODE
                if isscimlstructure(p)
                    pf = SciMLBase.ParamJacobianWrapper(
                        (du, u, p_inner, t)->unwrappedf(du, u, repack(p_inner), t), _t, y)
                else
                ...
                if isscimlstructure(p)
                    pf = RODEParamJacobianWrapper(
                        (du, u, p_inner, t, W)->unwrappedf(du, u, repack(p_inner), t, W),
                        _t, y, _W)
                else
        ...
        elseif autojacvec isa Bool
            if isinplace
                if SciMLBase.is_diagonal_noise(prob)
                    if isscimlstructure(p)
                        pf = SciMLBase.ParamJacobianWrapper(
                            (du, u, p_inner, t)->unwrappedf(du, u, repack(p_inner), t),
                            _t, y)
                    else
                        pf = SciMLBase.ParamJacobianWrapper(unwrappedf, _t, y)
                    end
