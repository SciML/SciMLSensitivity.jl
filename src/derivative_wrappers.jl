struct SensitivityAlg{CS,AD,FDT} <: DEAlgorithm
  diff_type::FDT
end
Base.@pure function SensitivityAlg(;chunk_size=0,autodiff=true,diff_type=Val{:central})
  SensitivityAlg{chunk_size,autodiff,typeof(diff_type)}(diff_type)
end
...
function jacobian!(J::AbstractMatrix{<:Number}, f, x::AbstractArray{<:Number}, fx::AbstractArray{<:Number}, alg::SensitivityAlg, jac_config)
    if alg_autodiff(alg)
  ...
  if alg_autodiff(alg)
    ...
    if alg.diff_type != Val{:complex}
      jac_config = DiffEqDiffTools.JacobianCache(similar(u),similar(u),
                                                 similar(u),alg.diff_type)
    else
      tmp = Complex{eltype(u)}.(u)
      du1 = Complex{eltype(u)}.(du1)
      jac_config = DiffEqDiffTools.JacobianCache(tmp,du1,nothing,alg.diff_type)
    end
  end
  jac_config
end
