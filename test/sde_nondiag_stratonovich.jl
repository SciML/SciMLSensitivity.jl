...
@testset "Non-diagonal noise tests" begin
    ...
    Random.seed!(seed)
    prob = SDEProblem(
        f_nondiag!, g_nondiag!, u₀, trange, p,
    ...
    @test isapprox(res_sde_u0a, res_sde_u0, rtol = 1.0e-6)
    @test isapprox(res_sde_pa, res_sde_p, rtol = 1.0e-5)
    ...
    @test all(
        (
            isapprox(res_sde_u0a, res_sde_u0, rtol = 1.0e-6),
            isapprox(res_sde_pa, res_sde_p, rtol = 1.0e-6),
        )
    )
    ...
    @test all(
        (
            isapprox(res_sde_u0a, res_sde_u0, rtol = 1.0e-5),
            isapprox(res_sde_pa, res_sde_p, rtol = 1.0e-4),
        )
    )
    ...
    @test isapprox(res1, res_sde_u0, rtol = 1.0e-4)
    @test isapprox(res2, res_sde_p', rtol = 1.0e-4)
end
...
@testset "diagonal but mixing noise tests" begin
    ...
    @test isapprox(res_sde_u0a, res_sde_u0, rtol = 1.0e-5)
    @test isapprox(res_sde_pa, res_sde_p, rtol = 1.0e-5) # would pass with 1e-4 but last noise value is off
    ...
    @test isapprox(res_sde_p', res_sde_forward, rtol = 1.0e-5)
    ...
    @test isapprox(res_sde_forward, res_sde_u0, rtol = 1.0e-5)
end
...
@testset "mixing noise inplace/oop tests" begin
    ...
    @test sol.u ≈ soloop.u atol = 1.0e-14
    ...
    @test res_sde_u0 ≈ res_sde_u02 atol = 1.0e-14
    @test res_sde_p ≈ res_sde_p2 atol = 1.0e-14

    @show res_sde_u0

    adjproboop = SDEAdjointProblem(
        soloop,
        BacksolveAdjoint(
            autojacvec = ZygoteVJP(),
            noisemixing = true
    ...
    adj_soloop = solve(
        adjproboop, EulerHeun(); dt = dtmix, tstops = soloop.t,
        adaptive = false
    )
    ...
    @test res_sde_u0 ≈ res_sde_u02 atol = 1.0e-8
    @test res_sde_p ≈ res_sde_p2 atol = 5.0e-8
...
@testset "mutating non-diagonal noise" begin
    ...
    @test dp1 ≈ _dp1 rtol = 1.0e-8
    @test dp2 ≈ _dp2 rtol = 1.0e-8
    @test dp3 ≈ _dp3 rtol = 1.0e-8
    @test_broken dp3 ≈ _dp4 rtol = 1.0e-8
end
