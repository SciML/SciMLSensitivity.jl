...
# These tests use Zygote extensively (Zygote.gradient, Zygote.ChainRules, etc.)
# and are skipped on Julia 1.12+ due to Zygote compatibility issues
if VERSION >= v"1.12"
    @info "Skipping MTK gradient tests on Julia 1.12+ due to Zygote compatibility issues"
    @testset "MTK Gradient Tests (skipped on Julia 1.12+)" begin
        @test_skip false
    end
else
