...
@testset "noise iip tests" begin
    ...
    sol2 = solve(
        remake(prob, noise = noise, tspan = (tspan[1], tspan[2])), RandomEM(),
        dt = dt, saveat = t
    )
    ...
    sol = solve(
        prob_aug, RandomEM(), dt = dt, save_noise = false, dense = false,
        saveat = t
    )
    du0,
        dp = adjoint_sensitivities(
...
@testset "noise oop tests" begin
    ...
    sol = solve(
        prob_aug, RandomEM(), dt = dt, save_noise = false, saveat = t,
        dense = false
    )
    du0,
        dp = adjoint_sensitivities(
